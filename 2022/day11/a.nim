type
    Monkey = object
        items: seq[int]
        operation: proc(worryLevel: int): int
        testDivisible: int
        testTrueDestination: int
        testFalseDestination: int
        itemsInspected: int

proc addItem(self: var Monkey, item: int) =
    self.items.add(item)

proc popItem(self: var Monkey): int =
    if self.items.len == 0:
        return -1
...
proc getMonkeyToThrowTo(self: Monkey, item: int): int =
...
monkeys[0] = Monkey(
    items: @[54, 53],
    operation: proc(worryLevel: int): int = worryLevel * 3,
    testDivisible: 2,
...
monkeys[1] = Monkey(
    ...
    operation: proc(worryLevel: int): int = worryLevel * 11,
    testDivisible: 7,
...
monkeys[2] = Monkey(
    ...
    operation: proc(worryLevel: int): int = worryLevel + 6,
    testDivisible: 3,
...
monkeys[3] = Monkey(
    items: @[83, 85, 85, 63],
    operation: proc(worryLevel: int): int = worryLevel + 4,
    testDivisible: 11,
...
monkeys[4] = Monkey(
    items: @[85, 52, 64],
    operation: proc(worryLevel: int): int = worryLevel + 8,
    testDivisible: 17,
...
monkeys[5] = Monkey(
    items: @[57],
    operation: proc(worryLevel: int): int = worryLevel + 2,
    testDivisible: 5,
...
monkeys[6] = Monkey(
    ...
    operation: proc(worryLevel: int): int = worryLevel * worryLevel,
    testDivisible: 13,
    testTrueDestination: 2,
...
monkeys[7] = Monkey(
    ...
    operation: proc(worryLevel: int): int = worryLevel + 5,
    testDivisible: 19,
...
echo maxInspections * nextMaxInspections