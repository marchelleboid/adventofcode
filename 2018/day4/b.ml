...
let accumulate_minutes index accumulator minutes_slept =
  let (_, sleepiest_minute_value) = accumulator in
...
let handle_guard ~key ~data accumulator =
  let (sleepiest_minute, sleepiest_minute_value) =
    Array.foldi ~init:(0,0) ~f:accumulate_minutes data
  in
    let (_, _, highest_sleepiest_minute_value) = accumulator in
      if sleepiest_minute_value > highest_sleepiest_minute_value then
      (key, sleepiest_minute, sleepiest_minute_value)
      else
      accumulator
...
let () =
  ...
  in
    let (guard_id, sleepiest_minute, _) = find_biggest_sleeper sleep_state.sleep_map in
