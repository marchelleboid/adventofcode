...
let solver =
    let lines = File.ReadLines "Inputs\\day18.txt" |> Seq.map Seq.toArray |> Seq.map (fun l -> Array.map (fun c-> System.String [|c|]) l)
    let starter = Seq.head lines
    lines
        |> Seq.skip 1
        |> Seq.fold (fun state line -> reduce (Array.concat [ [|"["|]; state; [|","|]; line; [|"]"|] ])) starter
        |> magnitude
        |> printfn "%d"