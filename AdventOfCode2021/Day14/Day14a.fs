...
let rec templateToCounts template counts =
    match template with
    | [] -> counts
    | a :: [] -> counts |> updateMapWithChar a 1
    | a :: b :: rest ->
        counts
            |> updateMapWithPair a b 1
            |> updateMapWithChar a 1
...
let solver =
    ...
    Seq.skip 2 lines
        ...
        |> performInsertions (templateToCounts (Seq.toList template) Map.empty<string, int>) 10
