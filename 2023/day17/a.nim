...
while len(queue) > 0:
    ...
    if current.x == len(grid[0]) - 1 and current.y == len(grid) - 1 and current.straights >= 0:
    ...
    if xDiff == 0:
        ...
        if current.straights >= 0 or current.straights == 0:
            # Go left
            maybeAddNewPath(current.x - 1, current.y, current.x, current.y, 1, current.heatLoss)
            # Go right
            maybeAddNewPath(current.x + 1, current.y, current.x, current.y, 1, current.heatLoss)
        # Go straight
        if current.straights < 3 and current.straights != 0:
